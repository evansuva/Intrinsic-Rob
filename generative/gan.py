...
class GAN(object):
    ...
    def get_lipschitz(self):
        ...
        log = open(self.save_dir+'/'+self.dataset+'/'+self.model_name+'/GAN_lipschitz.txt', "w")
        ...
        print("""(95%%) lipschitz: %.2f, (99%%) lipschitz: %.2f, (99.9%%) lipschitz: %.2f, (max) lipschitz: %.2f""" % 
                (np.percentile(L, q=95), np.percentile(L, q=99), np.percentile(L, q=99.9), np.max(L)))

        print("%.2f, %.2f, %.2f, %.2f" % (np.percentile(L, q=95), np.percentile(L, q=99), np.percentile(L, q=99.9), np.max(L)), file=log)
        log.flush()
    ...
    def reconstruct(self):
        ...
        np.save(data_dir+'/samples_train', samples_train)
        ...
        np.save(data_dir+'/samples_test', samples_test)

        samples_test = samples_test.transpose(0, 2, 3, 1)
        utils.save_images(samples_test[:100, :, :, :], [10, 10],
                          self.save_dir+'/'+self.dataset+'/'+self.model_name+'/GAN_gen_img.png')
