...
def train_baseline(loader, model, opt, epoch, log1, log2, verbose):
    ...
    for i, (X,y) in enumerate(loader):
        ...
        print(epoch, i, '{:.4f}'.format(err.item()), '{:.4f}'.format(ce.item()), file=log1)
    ...
    print(epoch, '{:.2%}'.format(errors.avg), '{:.4f}'.format(losses.avg), file=log2)
    log2.flush()  
...
def train_robust(loader, model, opt, epsilon, epoch, log1, log2, verbose, clip_grad=None, **kwargs):
    ...
    for i, (X,y) in enumerate(loader):
        ...
        print(epoch, i, '{:.2%}'.format(err), '{:.2%}'.format(robust_err),
             '{:.4f}'.format(ce), '{:.4f}'.format(robust_ce.detach().item()), file=log1)
        log1.flush()
...
## robust training for imporved robustness using lipschitz
def train_lipschitz_robust(loader, model, opt, epsilon, epoch, log1, log2, verbose, clip_grad=None, **kwargs):
    batch_time = AverageMeter()
    losses = AverageMeter()
    errors = AverageMeter()
    robust_losses = AverageMeter()
    ...
    for i, (X,y,lip) in enumerate(loader):
        X,y = X.cuda(), y.cuda().long()
        if y.dim() == 2: 
            y = y.squeeze(1)
        # data_time.update(time.time() - end)
        ...
        robust_ce, robust_err = robust_loss_lipschitz(model, epsilon, X, y, lip, **kwargs)
...
def train_robust_task_spec(loader, model, opt, epsilon, epoch, log1, log2, verbose, 
    ...
    print(epoch, '{:.2%}'.format(errors.avg), '{:.2%}'.format(robust_cost_avg), 
            '{:.4f}'.format(robust_losses.avg), file=log2)
    log2.flush()  
    torch.cuda.empty_cache()
...
def evaluate_test_clas_spec(loader, model, epsilon, path, verbose, **kwargs): 
    ...
    for i, (X,y) in enumerate(loader):
        ...
        if verbose: 
            endline = '\n' if i % verbose == 0 else '\r'
            print('Batch: [{:3d}/{}]'.format(i, len(loader)), end=endline)
...
## define the training loss function for lipschitz_robust training 
def robust_loss_lipschitz(net, epsilon, X, y, lip, **kwargs):

    f = RobustBounds(net, epsilon, **kwargs)(X,y)
    err = (f.max(1)[1] != y).sum().item()/X.size(0)

    # associate weights to examples using lipschitz constant 

    # weight_vec = torch.FloatTensor(np.power(lip, -10).numpy()).cuda()
    # weight_sum = np.sum(np.power(lip, -10).numpy())

    weight_normal = torch.FloatTensor((lip>np.percentile(lip, q=80)).numpy()).cuda()
    weight_robust = torch.FloatTensor((lip<=np.percentile(lip, q=80)).numpy()).cuda()

    ce_loss = torch.dot(nn.CrossEntropyLoss(reduction='none')(net(X), y), weight_normal) / torch.sum(weight_normal) 
    ce_loss += torch.dot(nn.CrossEntropyLoss(reduction='none')(f, y), weight_robust) / torch.sum(weight_robust)
