...
def custom_mnist_loaders(batch_size, train_path, test_path, ratio=0, 
                            seed=None, is_shuffle=False, is_lipschitz=False):

    if is_lipschitz:
        mnist_train_total = CustomLipschitzDataset(train_path, 28, 28, 1, transforms.ToTensor())
        mnist_test = CustomLipschitzDataset(test_path, 28, 28, 1, transforms.ToTensor())
    else:
        mnist_train_total = CustomDataset(train_path, 28, 28, 1, transforms.ToTensor())
        mnist_test = CustomDataset(test_path, 28, 28, 1, transforms.ToTensor())
    
    if ratio == 0:
        train_loader = td.DataLoader(dataset=mnist_train_total, batch_size=batch_size, shuffle=True, pin_memory=True)
...
def argparser(prefix=None, method=None, batch_size=50, batch_size_test=50,
              epochs=60, verbose=200, lr=1e-3, thres=None, ratio=0.2,
              seed=0, epsilon=0.1, starting_epsilon=0.05, 
              schedule_length=20, proj=None, norm_train='l1', 
              norm_test='l1', opt='sgd', momentum=0.9, weight_decay=5e-4): 
    ...
    if args.prefix:
        ...
        banned = ['proctitle', 'prefix', 'model', 'method', 'opt',
                  ...
                  'batch_size_test', 'lr', 'type', 'category', 'tuning', 
                  'starting_epsilon', 'schedule_length',
                  'ratio', 'seed', 'verbose', 'cuda_ids']
     
        if args.method == 'baseline':
            banned += ['thres', 'proj', 'norm_test', 'norm_train']
